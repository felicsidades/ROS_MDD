...
:: Generated by MPS
::----------------------------------------------------------------------
SET IDEA_PATHS_SELECTOR=ROSMDD1.0
SET PRODUCT=rosmdd
...
IF "%IDE_BIN_DIR:~-8%" == "bin\win\" (
  echo.
  echo    You are trying to run ROSMDD from generic distribution
  echo Please first copy content of "%IDE_BIN_DIR%"
  echo into its parent directory ("%IDE_BIN_DIR:~0,-4%"^)
  echo and then run rosmdd.bat from there
  echo.
  pause
  exit
)
...
:: Try (in order): ROSMDD_JDK, rosmdd%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=
SET IDEA_VENDOR_NAME=null

IF EXIST "%ROSMDD_JDK%" SET JDK=%ROSMDD_JDK%
IF EXIST "%JDK%" GOTO check

SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\ROSMDD1.0\rosmdd%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\ROSMDD1.0\rosmdd%BITS%.exe.jdk
...
IF NOT EXIST "%JAVA_EXE%" (
  ECHO ERROR: cannot start ROSMDD.
  ECHO No JDK found. Please validate either ROSMDD_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%ROSMDD_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%ROSMDD_PROPERTIES%"

SET VM_OPTIONS_FILE=
SET USER_VM_OPTIONS_FILE=
IF NOT "%ROSMDD_VM_OPTIONS%" == "" (
  :: 1. %<IDE_NAME>_VM_OPTIONS%
  IF EXIST "%ROSMDD_VM_OPTIONS%" SET "VM_OPTIONS_FILE=%ROSMDD_VM_OPTIONS%"
)
